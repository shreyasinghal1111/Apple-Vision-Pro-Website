...
gsap.to("#page-bottom",{
  ScrollTrigger:{
    ...
    scroller:"#main",
    scrub:.5,//Scrub is just the duration of the tween that is used to animate to whatever value the animation to its "current" values. So if your animation is creating a parallax effect, it could control the "speed" depending on what you mean by speed
  },
  opacity:0.7
})
...
function scaleImage(img, ctx) {
  ...
  var ratio = Math.min(hRatio, vRatio);//minimum isley use kra because images ki width kam h aur height zyada h
